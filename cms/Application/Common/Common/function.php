...
//保存BASE64图片
function mc_save_img_base64($img,$thumb=false) {
	if(mc_user_id()) {
		...
		if($attached_type!='') {
			...
			if(mc_option('upyun')==2) {
				$path = C('UPLOAD_CLOUD_PATH').'/img/'.$ymd.'/'.$out_trade_no.'.'.$attached_type;
				require_once(COMMON_PATH.'Common/upyun.class.php');
				$upyun = new UpYun(mc_option('upyun_bucket'), mc_option('upyun_user'), mc_option('upyun_pwd'));
			    $rsp = $upyun->writeFile($path, $img_decode, True);
			    $file_url = mc_option('upyun_url').$path;
			} else {
				...
				if (!file_exists($fileName)) { // 如果不存在则创建
					...
					if($thumb) {
					    ...
					    $image->open($fileName);
					    $image->thumb(400, 400,\Think\Image::IMAGE_THUMB_CENTER)->save($fileName);
				    };
				    */
				};
...
function mc_del_img($url){
    if(mc_option('upyun')==2) {
       	$path = str_replace(mc_option('upyun_url'), '', $url);
    	require_once(COMMON_PATH.'Common/upyun.class.php');
       	$upyun = new UpYun(mc_option('upyun_bucket'), mc_option('upyun_user'), mc_option('upyun_pwd'));
       	$upyun->deleteFile($path);
    }else{
    	$path = str_replace(mc_site_url().'/', '', $url);
    	unlink($path);
    }
};
//替换正文BASE64图片链接
...
function mc_views_count($id) {
	$meta['page_id'] = $id;
	$meta['meta_key'] = 'views';
	$views = M('count')->where($meta)->getField('meta_value');
	if($views) {
		return $views;
	} else {
		return 0;
...
//设置page阅读数
function mc_set_views($id,$type='basic') {
	$meta['page_id'] = $id;
	$meta['meta_key'] = 'views';
	$meta['type'] = $type;
	$views = mc_views_count($id);
	if($views) {
		$update['meta_value'] = $views+1;
		M('count')->where($meta)->save($update);
	} else {
		$meta['meta_value'] = 1;
		M('count')->data($meta)->add();
	}
};
//获取term下page数量
...
function mc_pagenavi($count,$page_now,$size=false) {
	...
	$page_url_on = mc_site_url().U(MODULE_NAME.'/'.CONTROLLER_NAME.'/'.ACTION_NAME);
	if(C('URL_MODEL')==0) {
		if($_GET['id']) {
...
function mc_prev_page_id($id) {
	if(is_numeric($id)) {
		$type = mc_get_page_field($id,'type');
		$page_id = M('page')->where("type='$type' AND id<'$id'")->order('id desc')->limit('0,1')->getField('id');
		return $page_id;
	};
};
//下一篇文章
function mc_next_page_id($id) {
	if(is_numeric($id)) {
		$type = mc_get_page_field($id,'type');
		$page_id = M('page')->where("type='$type' AND id>'$id'")->order('id asc')->limit('0,1')->getField('id');
		return $page_id;
	};
};
//--------------- PAGE ---------------//
		...
		foreach($parameter as $par) :
			...
			if($pro_parameter) :
				...
				foreach($pro_parameter as $pro_par) :
					$num_now++;
					if($num_now==1) :
						$kucun = M('meta')->where("page_id='$id' AND meta_key='$pro_par' AND type ='kucun'")->getField('meta_value');
						array_push($kucun_now,$kucun);
					endif;
				endforeach;
				$pos = array_search(min($kucun_now), $kucun_now);
				$kucun_pro = $kucun_now[$pos];
			else :
				$kucun_pro = mc_get_meta($id,'kucun');
			endif;
		endforeach;
	else :
		$kucun_pro = mc_get_meta($id,'kucun');
...
//--------------- USER ---------------//
//发送邮件
require_once (COMMON_PATH.'Common/formail.php');

//--------------- go -----------------//
require_once (COMMON_PATH.'Common/class-go.php');
require_once (COMMON_PATH.'Common/go.php');
//--------------- go -----------------//
//------------- weixin ---------------//
require_once (COMMON_PATH.'Common/function_weixin.php');
//------------- weixin ---------------//
?>